...
<head>
    ...
    <script>
        mudarStatus = (id, status) => {
            window.location.href = '../controller/mudarStatusCanal.act.php?id=' + id + '&status=' + status;
        }

         editar = (id) => {
            window.location.href = '../view/editarCanal.php?id=' + id;
        }
                ...
                <div class="admin-table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            $busca = mysqli_query($con, "SELECT * FROM `canais` ");



                            while ($canais = mysqli_fetch_assoc($busca)) {

                            ?>
                                <tr>
                                    <td class="name-cell"><?= $canais['nomeCanal'] ?></td>
                                    ...
                                    <td class="actions-cell">
                                         ...
                                         <?php 
                                            if($canais['status'] == 0){
                                                echo '<button class="admin-button small outline" onclick="mudarStatus('.$canais['idCanal'].', '.$canais['status'].')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg> Ativar</button>';
                                            }else{
                                                echo '<button class="admin-button small destructive" onclick="mudarStatus('.$canais['idCanal'].','.$canais['status'].')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg> Inativar</button>';
                                            }
                                        
                                        ?>
                                        
                                        <button class="admin-button small outline" onclick="editar(<?=$canais['idCanal']?>)">
