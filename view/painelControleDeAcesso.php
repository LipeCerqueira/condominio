...
<head>
    ...
    <script>
        mudarStatus = (id, status, local) => {
            window.location.href = '../controller/mudarStatus.act.php?id=' + id + '&status=' + status + '&local=' + local;
        }
        editar = (id, status) => {
            window.location.href = '../view/acessoUsuario.php?id=' + id;
        }
    </script>
</head>
    ...
    <div class="container">
        <div class="admin-card">
            <div class="admin-card-header">
                ...
                <a href="painelAssociacao.php" class="admin-button outline">Voltar</a>
            </div>
                ...
                <div class="admin-table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            $usuarios = mysqli_query($con, "SELECT * FROM `usuario` where nivel < 3");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="actions-cell">
                                        ...
                                        <?php
                                        if ($usuario['status'] == 0) {
                                            echo '<button class="admin-button small outline" onclick="mudarStatus(' . $usuario['id'] . ', ' . $usuario['status'] . ', \'associacao\')">Ativar</button>';
                                        } else {
                                            echo '<button class="admin-button small destructive" onclick="mudarStatus(' . $usuario['id'] . ', ' . $usuario['status'] . ', \'associacao\')">Inativar</button>';
                                        }
