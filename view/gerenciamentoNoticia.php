...
<head>
    ...
    <script>
        mudarStatusProfissional = (id, status) => {
            window.location.href = '../controller/mudarStatusNoticia.act.php?id=' + id + '&status=' + status;
        }
        editar = (id, status) => {
            window.location.href = '../view/editarNoticia.php?id=' + id;
        }
    </script>
</head>
    ...
    <div class="container">
        <div class="admin-card">
            <div class="admin-card-header">
                ...
                <a href="adicionarNoticia.php" class="admin-button outline">Adicionar Notícia</a>
                ...
                <div class="admin-table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            $noticias = mysqli_query($con, "SELECT *, DATE_FORMAT(dataNoticia, '%d/%m/%Y') as data_formatada FROM `noticia` ");
                            ...
                            ?>
                                <tr>
                                    <td class="name-cell"><?= $noticia['manchete'] ?></td>
                                    ...
                                    <td class="actions-cell">
                                        <button class="admin-button small outline" onclick="editar(<?=$noticia['idNoticia']?>)">
                                         ...
                                         <?php 
                                            if($noticia['status'] == 0){
                                                echo '<button class="admin-button small outline" onclick="mudarStatusProfissional('.$noticia['idNoticia'].', '.$noticia['status'].')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg> Ativar</button>';
                                            }else{
                                                echo '<button class="admin-button small destructive" onclick="mudarStatusProfissional('.$noticia['idNoticia'].','.$noticia['status'].')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg> Inativar</button>';
                                            }
