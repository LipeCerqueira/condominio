...
<head>
    ...
    <script>
        mudarStatus = (id, status) => {
            window.location.href = '../controller/mudarStatusCondominio.act.php?id=' + id + '&status=' + status;
        }
        editar = (id) => {
            window.location.href = '../view/editarCondominio.php?id=' + id;
        }
    </script>
</head>
                ...
                <div class="admin-table-container">
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            $condominios = mysqli_query($con, "SELECT * FROM `condominios` ");



                            while ($condominio = mysqli_fetch_assoc($condominios)) {

                            ?>
                                <tr>


                                    <td><?= $condominio['nome_fantasia'] ?></td>
                                    <td class="name-cell"><?= $condominio['razao_social'] ?></td>
                                    ...
                                    <td class="actions-cell">
                                        <button class="admin-button small outline" onclick="editar(<?= $condominio['id'] ?>)">
                                        ...
                                        if ($condominio['status'] == 'Ativo') {
                                            echo '<button class="admin-button small destructive" onclick="mudarStatus(' . $condominio['id'] . ', \'Ativo\')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon">
                    <path d="M12 20h9"></path>
                    <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
                </svg> Inativar</button>';
                                        } else {
                                            echo '<button class="admin-button small outline" onclick="mudarStatus(' . $condominio['id'] . ', \'Inativo\')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="button-icon">
                    <path d="M12 20h9"></path>
                    <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
                </svg> Ativar</button>';
